...
//     // appointment-service.ts (updated bookAppointment method)
// async bookAppointment(appointmentData: {
//     patientNumber: string;
//     doctorUniqueId: string;
//     diseaseOrHealthIssue: string;
...
//         // Update patient's appointments (using array instead of single field)
//         await PatientModel.findByIdAndUpdate(patient._id, {
//             $push: { appointments: appointmentUniqueId }
//         });
...
export class AppointmentService {
    // Book appointment (by patient) - REMOVED availability checks
    async bookAppointment(appointmentData: {
        patientNumber: string;
        doctorUniqueId: string;
        diseaseOrHealthIssue: string;
        ...
        try {
            ...
            // Find doctor (only check existence, not availability)
            const doctor = await DoctorModel.findOne({ 
                uniqueId: appointmentData.doctorUniqueId, 
                isDeleted: false 
            });
    ...
    // Update appointment (by patient) - REMOVED availability checks
    async updateAppointment(appointmentUniqueId: string, updateData: {
        diseaseOrHealthIssue?: string;
        appointmentDate?: Date;
        appointmentTime?: string;
    ...
    async cancelAppointment(appointmentUniqueId: string): Promise<boolean> {
        try {
            ...
            await PatientModel.findOneAndUpdate(
                { phoneNumber: appointment.patientNumber },
                { $pull: { appointments: appointmentUniqueId } }
            );
